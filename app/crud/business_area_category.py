...
def get_all_b_a_c_sub_category_by_main_category_code(
    ...
    try:
        ...
        print(f"get_all_b_a_c_sub_category_by_main_category_code Error: {e}")
        raise HTTPException(status_code=500, detail="Database query failed")
    finally:
        close_connection(connection)
...
def get_all_b_a_c_detail_category_by_sub_category_code(
    ...
    try:
        ...
        print(f"get_all_b_a_c_detail_category_by_sub_category_code Error: {e}")
        raise HTTPException(status_code=500, detail="Database query failed")
    finally:
        close_connection(connection)
...
def select_all_b_a_c_category_by_dynamic_query(
    ...
    try:
        if connection.open:
            ...
            select_query += " ORDER BY BUSINESS_AREA_CATEGORY_ID DESC"
            ...
            for row in rows:
                category_output = CategoryListOutput(
                    category_id=row.get("BUSINESS_AREA_CATEGORY_ID"),
