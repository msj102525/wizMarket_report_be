...
CREATE TABLE
    `BIZ_DETAIL_CATEGORY_CONTENT` (
        ...
        `CONENT` TEXT NOT NULL COMMENT '내용',
...
CREATE TABLE
    `BIZ_DETAIL_CATEGORY_CONTENTIMAGE` (
...
CREATE TABLE
    `LOCAL_SOTRE_CONTENT` (
        `LOCAL_SOTRE_CONTENT_ID` INT AUTO_INCREMENT PRIMARY KEY COMMENT 'PK_LSCID',
        `STORE_BUSINESS_NUMBER` VARCHAR(100) NOT NULL COMMENT 'FK_RID',
        `DETAIL_CATEGORY_ID` INT NOT NULL COMMENT '비즈맵 소분류 ID',
        `TITLE` VARCHAR(255) NOT NULL COMMENT '제목',
        `CONENT` TEXT NOT NULL COMMENT '내용',
...
CREATE TABLE
    `LOCAL_SOTRE_CONTENT_IMAGE` (
        `LOCAL_SOTRE_CONTENT_IMAGE_ID` INT AUTO_INCREMENT PRIMARY KEY COMMENT 'PK_LSCIID',
        `LOCAL_SOTRE_CONTENT_ID` INT NOT NULL COMMENT 'FK_LSCID',
        `LOCAL_SOTRE_CONTENT_IMAGE_URL` VARCHAR(255) NOT NULL COMMENT '이미지 URL',
        `CREATED_AT` DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '생성일시',
        `UPDATED_AT` DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '수정일시',
        FOREIGN KEY (`LOCAL_SOTRE_CONTENT_ID`) REFERENCES `LOCAL_SOTRE_CONTENT` (`LOCAL_SOTRE_CONTENT_ID`) ON DELETE CASCADE
    );