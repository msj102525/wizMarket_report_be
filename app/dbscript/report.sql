...
CREATE TABLE
    `REPORT` (
        ...
        `LOC_INFO_DISTRICT_HOT_PLACE_TOP1_INFO` VARCHAR(100) NULL COMMENT '입지분석 시/군/구 핫플레이스 TOP5 1등 (지역, 평균유동인구, 매장평균매출, JSCORE점수)',
        `LOC_INFO_DISTRICT_HOT_PLACE_TOP2_INFO` VARCHAR(100) NULL COMMENT '입지분석 시/군/구 핫플레이스 TOP5 2등 (지역, 평균유동인구, 매장평균매출, JSCORE점수)',
        `LOC_INFO_DISTRICT_HOT_PLACE_TOP3_INFO` VARCHAR(100) NULL COMMENT '입지분석 시/군/구 핫플레이스 TOP5 3등 (지역, 평균유동인구, 매장평균매출, JSCORE점수)',
        `LOC_INFO_DISTRICT_HOT_PLACE_TOP4_INFO` VARCHAR(100) NULL COMMENT '입지분석 시/군/구 핫플레이스 TOP5 4등 (지역, 평균유동인구, 매장평균매출, JSCORE점수)',
        `LOC_INFO_DISTRICT_HOT_PLACE_TOP5_INFO` VARCHAR(100) NULL COMMENT '입지분석 시/군/구 핫플레이스 TOP5 5등 (지역, 평균유동인구, 매장평균매출, JSCORE점수)',
...
CREATE TABLE
    `LOCAL_STORE_IMAGE` (
        ...
        FOREIGN KEY (`STORE_BUSINE SS_NUMBER`) REFERENCES `REPORT` (`STORE_BUSINESS_NUMBER`) ON DELETE CASCADE
    );
...
CREATE TABLE
    `ADS` (
        ...
        `DETAIL_TITLE` VARCHAR(255) NULL COMMENT '세부 주제',
        `CONTENT` TEXT NULL COMMENT '문구',
        `STATUS` CHAR(1) NOT NULL DEFAULT 'Y' COMMENT '게시 여부',
        `CREATED_AT` DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '생성일시',
        `UPDATED_AT` DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '수정일시'
    );
...
CREATE TABLE
    `ADS_IMAGE` (
        ...
        `ADS_FINAL_IMAGE_URL` VARCHAR(255) NULL COMMENT '최종 생성 이미지 URL',
        `ADS_IMAGE_URL` VARCHAR(255) NULL COMMENT '이미지 URL',
        `CREATED_AT` DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '생성일시',
        `UPDATED_AT` DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '수정일시',
